...
const UserCensus = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      await Api.post("/census/prevalidate", data)
        .then(async (res) => {
          setValidation(true)
          //console.log(alertResults)
          setRender(res.data.census);
        })
        .catch((error) => {
  ...
  const validationEmail = async () => {
    ...
    await Api.post("/census/validate", data)
      .then((res) => {
        setValidation(true)
        setRender(res.data.census);
      })
  ...
  const alertResults = useCallback(async (sender) => {
    ...
    await Api.post("/census/results", data)
      .then((res) => {
            ...
            <Card sx={{ py: 6, px: 5 }}>
              <CardContent>
                ...
                <Typography
                  ...
                  sx={{ pb: 1 }}
                >
                  Ingresa tu correo para censarte.
                </Typography>
                <TextField
                  variant="standard"
                  size="small"
