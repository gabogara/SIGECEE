...
const AddCensusModal = ({
  ...
  // Funcion que permite guardar el censo
    ...
    if (evalData.pages !== undefined) {
      if (evalData.title !== undefined) {
        if (evalData.description !== undefined) {
          ...
          await Api.post('/census/add', register_census)
            .then(async (res) => {
              cleanModal()
              setOpenSnack(true)
          ...
          setSnackMessage({
            ...snackMessage,
            color: 'warning',
            message: 'La descripción del censo está vacía. Debe ingresar una descripción.',
          })
        }
      } else {
        setOpenSnack(true)
      ...
      setSnackMessage({
        ...snackMessage,
        color: 'warning',
        message: 'El censo está vacío. Debe eligir una o más preguntas para guardar.',
      })
    }
    setProgress(false)
  }
...
export default AddCensusModal

AddCensusModal.propTypes = {
