...
const ResultNumberTable = ({
  ...
  useEffect(() => {
    (async () => {
      ...
      resultRows.push({
        ...
        result_count:
          (
            <Typography sx={{ fontSize: 16, fontWeight: '800' }}>Total: {finalCount}</Typography>
          ),
      })
      setRows(resultRows)
    })();
  ...
  const columns = [
    // { name: 'id', title: 'Id' },
    { name: 'result_number', title: 'Número' },
  ...
  const pagingMessages = {
    ...
    rowsPerPage: 'Números por página',
  }
...
ResultNumberTable.propTypes = {
  data: PropTypes.array,
}

export default ResultNumberTable
