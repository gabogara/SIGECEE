...
const ImportPopulation = ({
  ...
  const [{ name, errorEmptyName, description, errorEmptyDesc }, setState] =
  ...
  const Import = async () => {
    ...
    if (name.trim() !== "" && description.trim() !== "") {
      ...
      await Api.post("/population/add", add_population)
        .then(async (res) => {
          cleanModal();
          setProgress(false);
          setOpenSnack(true);
          ...
          <Box sx={{ width: "100%", marginTop: 2, marginBottom: 2 }}>
            ...
            >
              ...
              <Grid item xs={12} sx={{ mt: 2 }}>
                Descarga el ejemplo de la plantilla en Excel para poblaciones <Link href="/doc/Plantilla_poblaciones_masiva.xlsx" target="_blank" download>aquí</Link>.
              </Grid>
              <Grid item xs={12} sx={{ mt: 2 }}>
...
export default ImportPopulation;
