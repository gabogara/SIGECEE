...
const DeleteStructModal = ({
  ...
  const DeleteStruct = async (e) => {
    setProgress(true)
    var delete_struct = {
      _id: data._id,
    }
    await Api.post('/struct/delete', delete_struct)
      .then(async (res) => {
        setOpenSnack(true)
      ...
      <AppBar sx={{ position: 'relative', backgroundColor: '#0d47a1' }}>
        <Toolbar>
          <Typography sx={{ flex: 1 }} variant="h6" component="div">
            ¿Desea borrar la siguiente estructura base?
          </Typography>
          <IconButton
            edge="start"
            color="inherit"
      ...
      <Box sx={{ height: '100%' }}>
        <Grid container>
          <Grid item xs={12}>
            ...
            }} variant="extended" onClick={DeleteStruct}>
              <DeleteIcon sx={{ mr: 1 }} />
              Borrar
            </Fab>
          </Grid>
...
export default DeleteStructModal

DeleteStructModal.propTypes = {
